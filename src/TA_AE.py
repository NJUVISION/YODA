...
from src.rt_modules.layers.layers import DepthConvBlock
...
class FeatureExtractor(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(3, 128, 1),
            ResBlock(128, 128, norm_type='rms_norm', act_fn = 'silu'),
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(128, 256, 3, stride=2, padding=1),
            ResBlock(256, 256, norm_type='rms_norm', act_fn = 'silu'),
        )
        self.conv3 = nn.Sequential(
            nn.Conv2d(256, 512, 3, stride=2, padding=1),
            ResBlock(512, 512, norm_type='rms_norm', act_fn = 'silu'),
        )
        ...
        self.conv5 = nn.Sequential(
            ...
            ResBlock(1024, 1024, norm_type='rms_norm', act_fn = 'silu'),
        )
    def forward(self, x):
        ctx1 = self.conv1(x)
        ctx2 = self.conv2(ctx1)
        ctx3 = self.conv3(ctx2)
        ctx4 = self.conv4(ctx3)
        ctx5 = self.conv5(ctx4)
        return [ctx5, ctx4, ctx3, ctx2, ctx1]
...
class Encoder(nn.Module):
    def __init__(
        ...
        for i, (out_channel, num_layers) in enumerate(zip(block_out_channels, layers_per_block)):
            ...
            if i < num_blocks - 1 and num_layers > 0:
                ...
                fusion_block = DepthConvBlock(out_channel * 2, out_channel)
                adapter = nn.Conv2d(out_channel, out_channel, 3, 1, 1)
                adapter_blocks.append(adapter)
                # fusion_block  = get_block(
                #     block_type[i],
                #     out_channel * 2,
                #     out_channel,
                #     attention_head_dim=attention_head_dim,
        ...
        self.adapter_blocks = nn.ModuleList(adapter_blocks)
    ...
    def forward(self, hidden_states: torch.Tensor, ctxs: list[torch.Tensor]) -> torch.Tensor:
        ...
        for i, down_block in enumerate(self.down_blocks):
            ...
            if i < len(self.down_blocks) - 1:
                ctx = self.adapter_blocks[i](ctxs[i])
                hidden_states = torch.cat([hidden_states, ctx], dim=1)
                hidden_states = self.fusion_blocks[i](hidden_states)
...
class Decoder(nn.Module):
    def __init__(
        ...
        for i, (out_channel, num_layers) in reversed(list(enumerate(zip(block_out_channels, layers_per_block)))):
            ...
            if i < num_blocks - 1 and num_layers > 0:
                ...
                fusion_block = DepthConvBlock(out_channel * 2, out_channel)
                adapter = nn.Conv2d(out_channel, out_channel, 3, 1, 1)
                adapter_blocks_list.append(adapter)
                fusion_blocks_list.append(fusion_block)
            for _ in range(num_layers):
                block = get_block(
                    block_type[i],
                    out_channel,
            ...
            fusion_blocks.insert(0, nn.Sequential(*fusion_blocks_list))
            adapter_blocks.insert(0, nn.Sequential(*adapter_blocks_list))
    ...
    def forward(self, hidden_states: torch.Tensor, ctxs: list[torch.Tensor] = None) -> torch.Tensor:
        if self.in_shortcut:
        ...
        for i, (up_block, fusion_block, adapter_block) in enumerate(zip(reversed(self.up_blocks), reversed(self.fusion_blocks), reversed(self.adapter_blocks))):
            hidden_states = up_block(hidden_states)
            if i > 0:
                ctx = adapter_block(ctxs[i-1])
                hidden_states = torch.cat([hidden_states, ctx], dim=1)
                hidden_states = fusion_block(hidden_states)
...
class AutoencoderTA_AE(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    r"""
    ...
    def _encode(self, x: torch.Tensor, ctx: list[torch.Tensor]) -> torch.Tensor:
        batch_size, num_channels, height, width = x.shape

        if self.use_tiling and (width > self.tile_sample_min_width or height > self.tile_sample_min_height):
            return self.tiled_encode(x, return_dict=False)[0]

        encoded = self.encoder(x, ctx)

        return encoded

    @apply_forward_hook
    def encode(self, x: torch.Tensor, ctx: list[torch.Tensor], return_dict: bool = True) -> Union[EncoderOutput, Tuple[torch.Tensor]]:
        r"""
        ...
        if self.use_slicing and x.shape[0] > 1:
            ...
            encoded = self._encode(x, ctx)

        if not return_dict:
            return (encoded,)
        return EncoderOutput(latent=encoded)

    def _decode(self, z: torch.Tensor, ctx: list[torch.Tensor]) -> torch.Tensor:
        batch_size, num_channels, height, width = z.shape

        if self.use_tiling and (width > self.tile_latent_min_width or height > self.tile_latent_min_height):
            return self.tiled_decode(z, ctx, return_dict=False)[0]

        decoded = self.decoder(z, ctx)

        return decoded

    @apply_forward_hook
    def decode(self, z: torch.Tensor, ctx: list[torch.Tensor], return_dict: bool = True) -> Union[DecoderOutput, Tuple[torch.Tensor]]:
        r"""
    ...
    def forward(self, sample: torch.Tensor, last_recon: torch.Tensor, return_dict: bool = True) -> torch.Tensor:
        ctxs = self.decoder.feature_extractor(last_recon)
        encoded = self.encode(sample, ctxs=ctxs, return_dict=False)
        decoded = self.decode(encoded, ctxs=ctxs, return_dict=False)
        # if not return_dict:
        #     return (decoded,)
        return decoded
