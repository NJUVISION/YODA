...
class DMC(CompressionModel):
    def __init__(self):
        ...
        self.q_encoder = nn.Parameter(torch.ones((64 + extra_qp, g_ch_d, 1, 1))) #72, 256, 1, 1
        self.q_decoder = nn.Parameter(torch.ones((64 + extra_qp, g_ch_d, 1, 1)))
        self.q_feature = nn.Parameter(torch.ones((64+ extra_qp, g_ch_d, 1, 1)))
        self.q_recon = nn.Parameter(torch.ones((64 + extra_qp, g_ch_recon, 1, 1)))
    ...
    def compress(self, x, qp):
        ...
        #他下采样相当于把 [B, 3, H, W] 变成 [B, 3 * 8^2, H // 8, W //8]]
        #如果refer特征不存在，使用rgb/yuv提取特征，如果特征存在，则直接提取特征
        feature = self.apply_feature_adaptor()
        #ctx是不经过可变码率的量化卷积得到的先验，ctx_t是量化的只经过两层卷积的先验
        ctx, ctx_t = self.feature_extractor(feature, q_feature)
        #在进入第二层卷积之前torch.cat(feature, ctx), 第三层卷积之后进行自适应量化
    ...
    def forward(self, x):
        ...
        q_encoder = self.get_qp(qp, self.q_encoder)
        q_decoder = self.get_qp(qp, self.q_decoder)
        q_feature = self.get_qp(qp, self.q_feature)
        q_recon = self.get_qp(qp, self.q_recon)
        feature = self.apply_feature_adaptor()
        ctx, ctx_t = self.feature_extractor(feature, q_feature)
        # plot_gaussian_curve(ctx)
        y = self.encoder(x, ctx, q_encoder)
        ...
        x_hat, feature = self.get_recon_and_feature(y, ctx, q_decoder, q_recon)
        self.add_ref_frame(feature, None)
        ...
        bits_z = self.get_z_bits(z_hat, self.bit_estimator_z, qp)
        bpp_y = torch.sum(bits_y, dim=(1, 2, 3)) / pixel_num
        bpp_z = torch.sum(bits_z, dim=(1, 2, 3)) / pixel_num
        bpp = bpp_y + bpp_z
        bit = torch.sum(bpp) * pixel_num

        return  {"x_hat": x_hat, "bpp_y": bpp_y, "bpp_z": bpp_z}
