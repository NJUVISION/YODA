...
from src.rt_modules.layers.cuda_inference import CUSTOMIZED_CUDA_INFERENCE, round_and_to_int8
...
class DMCI(CompressionModel):
    ...
    def forward(self, x, qp):
        ...
        curr_q_enc = self.get_qp(qp, q_scale_model=self.q_scale_enc)
        curr_q_dec = self.get_qp(qp, q_scale_model=self.q_scale_dec)
        qp_tensor = self.get_q_tensor(qp, device=x.device)
        y = self.enc(x, curr_q_enc)
        ...
        # analyze_sparsity(z)
        # ddd
        # plot_gaussian_curve(y[0].detach().cpu().numpy(), save_path='./',frame_idx=0, layer_id=1)      
        # ddd
        params = self.hyper_dec(z_hat)
        params = self.y_prior_fusion(params)
        _, _, yH, yW = y.shape
        ...
        #     vutils.save_image(x_hat.float(), f'./test/perturb_{name}.png')
        # dddd
        
        bits_y = self.get_y_gaussian_bits(y_q, scales_hat)
        bits_z = self.get_z_bits(z_hat.to(dtype=torch.int8), self.bit_estimator_z, qp_tensor)
        _, _, H, W = x.size()
        pixel_num = H * W
